...
function buildQuery(params: ProductListParams = {}) {
  ...
  params.categoryIds?.forEach((categoryId) => searchParams.append('categoryIds', categoryId))
  if (params.page !== undefined) searchParams.set('page', String(params.page))
  if (params.size !== undefined) searchParams.set('size', String(params.size))
  if (params.sort && !params.sort.startsWith('price,')) searchParams.set('sort', params.sort)

  const query = searchParams.toString()
  return query ? `?${query}` : ''
}

export async function normalizeProductSummary(dto: ProductSummaryDto): Promise<ProductUi> {
  const image = await resolveMediaUrl(dto.primaryMedia?.s3Key, fallbackProductImage)
...
export async function normalizeProductDetail(dto: ProductDetailDto): Promise<ProductUi> {
...
export async function getProductsApi(params?: ProductListParams) {
  const page = await apiRequest<PageResponse<ProductSummaryDto>>(`/api/products${buildQuery(params)}`, {
    auth: false,
  })
...
export async function getProductApi(productId: string) {
  const product = await apiRequest<ProductDetailDto>(`/api/products/${encodeURIComponent(productId)}`, {
    auth: false,
  })
